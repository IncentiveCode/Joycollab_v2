...
{
    public class BookmarkSeminar : WindowView, iRepositoryObserver
    {
        private const string TAG = "BookmarkSeminar";
    ...
    #region WindowPage functions
        ...
        protected override void Init() 
        {
            ...
            _scrollView.AddSelectCallback((data) => {
                Debug.Log($"{TAG} | 해당 세미나 상세 화면으로 이동.");
                // Debug.Log($"{TAG} | 해당 세미나 상세 화면으로 이동. seminar seq : {((WorldAvatarData)data).info.seq}, name : {((WorldAvatarData)data).info.nickNm}");
            });
        }
