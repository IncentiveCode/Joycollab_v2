...
{
    public class BookmarkBoard : WindowView, iRepositoryObserver
    {
        private const string TAG = "BookmarkBoard";
    ...
    #region WindowPage functions
        ...
        protected override void Init() 
        {
            ...
            _scrollView.AddSelectCallback((data) => {
                Debug.Log($"{TAG} | 해당 게시글로 이동.");
                // Debug.Log($"{TAG} | 해당 게시글로 이동. board seq : {((WorldAvatarData)data).info.seq}, name : {((WorldAvatarData)data).info.nickNm}");
            });
        }
