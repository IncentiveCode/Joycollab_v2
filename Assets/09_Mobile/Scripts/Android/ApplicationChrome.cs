...
class ApplicationChrome {
	...
	// Constants
	private const uint DEFAULT_TOP_COLOR = 0xfffeb336;
	...
	// private static uint _statusBarColor = DEFAULT_TOP_COLOR;
		...
		using (var unityPlayer = new AndroidJavaClass("com.unity3d.player.UnityPlayer")) {
			using (var activity = unityPlayer.GetStatic<AndroidJavaObject>("currentActivity")) {
				using (var resources = activity.Call<AndroidJavaObject>("getResources")) {
					...
					int resId = resources.Call<int>("getIdentifier", "status_bar_height", "dimen", "android");
					if (resId > 0)
						return resources.Call<int>("getDimensionPixelSize", resId);
					else 
						return 0;
				}
			}
	...
	public static int GetNavigationBarHeight() {
		using (var unityPlayer = new AndroidJavaClass("com.unity3d.player.UnityPlayer")) {
			using (var activity = unityPlayer.GetStatic<AndroidJavaObject>("currentActivity")) {
				using (var resources = activity.Call<AndroidJavaObject>("getResources")) {
					string packageName = activity.Call<string>("getPackageName");
					int resId = resources.Call<int>("getIdentifier", "navigation_bar_height", "dimen", "android");
					if (resId > 0)
						return resources.Call<int>("getDimensionPixelSize", resId);
					else 
						return 0;
